...
That is, by the time it reaches the ground, the magnetic field from an FLR with $m=10$ is weaker by a factor of two; at $m=100$, the attenuation factor is closer to \num{e-2}. 
...
Both poloidal and toroidal waves are noted for their ability to contribute to the energization and radial diffufion of trapped particles. The poloidal mode interacts more strongly, since its electric field is aligned with the trapped particles' drift motion. Poloidally-polarized waves are also more prone to creating magnetic signatures on the ground, due to ducting in the ionosphere\cite{fujita_1988,greifinger_1968}. 

Toroidal modes have been shown to far outnumber poloidal modes\cite{anderson_1990}. Perhaps not coincidentally, poloidally-polarized waves rotate asymptotically to the toroidal mode\cite{mann_1995,mann_1997,radoski_1974}. Poloidal waves with low azimuthal modenumber -- such as those driven by broadband sources at the magnetopause -- rotate particularly quickly. 

\todo{Fishbone instability\cite{chen_1984,mcguire_1983}. Like the poloidal mode, but for lab plasmas. }

\todo{Poloidal and toroidal modes are coupled by the ionospheric Hall conductivity\cite{kato_1956}. }
...
all referred to ground phenomena. Over time, they have come to be used to describe satellite observations as well, including those not measured on Earth. }. In 2013, Takahashi described a strong, sinusoudal, high-\azm, fundamental poloidal resonance; however, because there was no associated ground signature, 
...
\todo{It bears noting, as was mentioned in \cref{sec_azm}, that large-modenumber waves are screened by the ionosphere. The fact that Pgs are typically observed with $m \lesssim 40$ does not necessarily indicate that strong, sinusoidal, odd-harmonic, poloidal Pc4s aren't happening in space. }
...
\todo{Why build a model? }

There's lots of observation. 

Even multisatellite events give a limited big-picture view. 

Analytical work often takes $\azm\rightarrow0$ or $\azm\rightarrow\infty$, or neglects the obtuse angle between the field line and the ionosphere, or is limited to a single frequency. 

The consideration of large-but-finite \azm is interesting! Rotation rate depends on \azm. Ground signatures depend on \azm. 

A model gives a broader view of the wave's behavior as it spans the inner magnetosphere. Allow for a unification of previously-disparate results. 

\todo{Why this model? }

Global models have no plasmasphere, ionosphere. 

3D models can't do large modenumber. 

\todo{What can we do once we have the model? }

Effect of the ionosphere. Why don't we see drift-resonant interactions on the nightside? 

How weird are giant pulsations? Under what conditions might sinusoidal waves be expected to arise? What do Pc4 ground signatures look like as a function of \azm, frequency, and ionospheric conditions? 

By its very definition, drift resonance has the potential to spur wave-particle interactions at all MLT. However, FLR observations are strongly peaked on the dayside. In his 2015 paper, Dai notes, ``It is not clear why noncompressional [high-\azm] Pc4 poloidal waves, which are presumably driven by instability within the magnetosphere, preferrentially occur on the dayside''\cite{dai_2015}. Motoba, in a paper written later that year, echoes, ``It is unclear whether other generation mechanisms of fundamental standing waves ... can explain the localization of Pgs in local time'' and adds that ``No theoretical study has explained why the occurrence of Pgs depends on solar activity''\cite{motoba_2015}. A goal of the present work is to investigate the problem numerically: to what degree is field line resonance affected by the conditions of the ionosphere? 

\todo{With that in mind, what data would be super helpful? }

It's been shown that a ground magnetometer \SI{66}{\degree} north of the magnetic equator observes \about10 events per year. It's also been shown that poloidal Pc4s are rare compared to toroidal ones, and that most poloidal Pc4s are even harmonics. However, little attention has been paid to how these rates line up with one another. Given the relative occurrence rate of poloidal and toroidal waves, of odd and even harmonics, and of diffuse and sharp spectral peaks, just how unusual are giant pulsations? 
