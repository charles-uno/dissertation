...
\todo{Early work. Ground signatures in the 1930s\cite{angenheister_1931}. Dungey\cite{dungey_1954}. Conjugate foot points\cite{sigura_1961}. Structure (?)\cite{nagata_1963}. }

\todo{Other planets\cite{glassmeier_2004}. }
...
In the present work, the wave in question is a harmonic resonance along a geomagnetic field line. It can also be envisioned as a superposition of traveling \Alfven waves, reflecting back and forth between its northern and southern foot points at the conducting ionosphere. These waves travel at the \Alfven speed\footnote{The \Alfven speed is given by \va is given by $\va^2 \equiv \frac{B^2}{\mz \rho}$, where $B$ is the magnitude of the magnetic field, \mz is the magnetic constant, and $\rho$ is the mass density of the ambient plasma. It can vary by several orders of magnitude over the length of a magnetic field line. }. 

In the inner magnetosphere, the \Alfven frequency\footnote{The \Alfven frequency is the inverse of the \Alfven bounce time: $\frac{2 \pi}{\omega_A} \equiv \oint \frac{dz}{v_A}$. } coincides with integer or half-integer multiples of particle drift frequencies\cite{dai_2013}, as discussed in \cref{sec_harmonics}. The resulting wave-particle interactions can give rise to significant energization and radial diffusion of the particles\todo{Cite... everybody? }. In some cases, the waves also include an electric field parallel to the background magnetic field, contributing to the precipitation of energetic particles into the neutral atmosphere. 

The present chapter discusses field line resonances (FLRs). Particular attention is paid to FLR spatial structure and polarization, how those parameters affect the behavior of the wave, and several unresolved questions related to that behavior. 
...
The wavelength of an FLR in the azimuthal direction is indicated by its azimuthal wavelength. A wave with modenumber \azm has an azimuthal wavelength that spans $\frac{24}{\azm}$ hours in MLT\footnote{MLT is Magnetic Local Time. Noon points toward the Sun and midnight away from it, with 06:00 and 18:00 at the respective dawn and dusk flanks. }. 
...
\todo{Toroidal modes are much more common. }
...
\todo{Poloidal modes are often accompanied by a mishmash of toroidal activity. }
