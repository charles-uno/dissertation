...
\todo{Also 2.5D: Waters 2013\cite{waters_2013}. }
...
\todo{Call radoski's coordinates x, y, and z (footnote an acknowledgement that he used different letters). }

\todo{table for vocabulary. xhat is radial. footnote in how that's the convention, since satellites mostly observe near the equator, even though it's very much not radial near the ionosphere. Or call it crosswise? }
...
%Covariant basis vectors $\hat{\varv}_i \equiv \dd{\lysaki} \vec{x}$ are normal to the curve defined by constant $\lysaki$. 

%Contravariant basis vectors $\hat{\varv}^i \equiv \dd{ \vec{x} } \lysaki$ are tangent to the coordinate curve. Equally, they're normal to the plane defined by constant $\lysakj$ for all $j \ne i$. 
...
\todo{Talk about linearized differentiation and interpolation here. That means we also talk about the leapfrog grid. Waters\cite{waters_2013} cites Taflove and Hagness, an electrodynamics textbook. ``leapfrog technique for time iteration.''}
