...
As long as we're looking at phenomena which are azimuthally localized, we should be fine. And, as we discussed in \cite{dai_2015}, Pc4 pulsations are localized in MLT. 
...
Note: this does not deform the field lines. It just renormalizes the coordinate we use to index from the northern foot point to the southern one. 
\begin{align}
  \label{def_coords}
...
% -----------------------------------------------------------------------------
% -----------------------------------------------------------------------------
\subsection{Metric Tensor and Jacobian Determinant}
...
Our model simulates the evolution of electric and magnetic fields in accordance with Maxwell's equations. Specifically, magnetic fields are advanced using \farlaw, and electric fields with \amplaw. We use Kirchhoff's formulation of \ohmlaw -- $\vec{J} = \tensor{\sigma} \cdot \vec{E}$ -- to eliminate the explicit current dependence in \amplaw. 
\begin{align}
  \ddt \vec{B} &= - \curl{E} &
...
\farlaw can be used to advance magnetic fields directly from the electric fields. In generalized coordinates, this is written
\begin{align}
...
DO WE WANT TO REALLY SPELL OUT WHAT THE COEFFICIENTS LOOK LIKE?
...
Neither the permittivity tensor nor the conductivity tensor couples the parallel and perpendicular components of \amplaw, so we can consider them separately. The perpendicular equation takes the form
\begin{align}
  \ep \ddt \vec{E}_\bot &= \frac{1}{\mu_0} \lr{ \curl{B} }_\bot - \tensor{\sigma}_\bot \cdot \vec{E}_\bot
\end{align}

If we cast this expression in terms of the linear polarizations $E_x$ and $E_y$, we find them to be coupled by the Hall conductivity. However, the conductivity tensor is be diagonal when we write this expression in terms of the circular polarizations. Taking $E_\pm \equiv \frac{1}{ \sqrt{2} } \lr{E_x \pm i E_y}$, and defining components of \curl{B} analogously, we have
\begin{align}
  \ep \ddt E_\pm &= \frac{1}{\mu_0} \lr{ \curl{B} }_\pm - \lr{ \sp \pm i \sh } E_\pm
\end{align}

The differential equations for the two perpendicular electric field components, now decoupled, can be solved using integrating factors. 
\begin{align}
  \label{eperp_tensor}
  \vec{E}_\bot^{ \mathrm{new} } &= 
...
Where $\va^2 \equiv \frac{1}{\mu_0 \ep}$ and $\tensor{R}$ is the rotation matrix
\begin{align}
...
The electric field values on the left and right side of \cref{eperp_tensor} are offset by a time step. The magnetic field, on the other hand, is off by half a time step. 

Derivatives can't easily be taken in terms of \x, \y, and \z, so we map this expression into our covariant basis.
\begin{align}
  \vec{E}_\bot^{ \mathrm{new} } &= 
  \tensor{R} \arg{ \tfrac{- \sh \dt}{\ep} } \cdot
  \vec{E}_\bot \exp \arg{ \tfrac{- \sp \dt}{\ep} } +
...
Where recall that $\lr{ \curl{B} }^i = \varepsilon^{ijk} \partial_j B_k$
...
\begin{align}
  \epsilon_\bot \ddt \vv{E_x}{E_y} &= \frac{1}{\mu_0} \vv{ \dd{y} B_z - \dd{z} B_y }{ \dd{z} B_x - \dd{x} B_z } - \mm{\sp}{-\sh}{\sh}{\sp} \cdot \vv{E_x}{E_y}
\end{align}

The perpendicular linear polarizations are coupled by the Hall conductivity. In order to obtain a pair of decoupled equations, we instead consider the circular polarizations, $E $



Awkwardly, $E_x$ and $E_y$ are coupled when the Hall conductivity is nonzero. 




The two perpendicular components are coupled by the Hall conductivity. This can be circumvented by rotating to a circularly-polarized basis, applying the integrating fac


This expression can be solved with integrating factors, th
