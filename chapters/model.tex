...
%### 5.1
%- Should there be a ':' before eq 5.2?
%- So many parenthesis, perhaps try em dashes, or commas (and replace the list
%  commas with semicolons).
%- "Four different for the low-density perpendicular" That is not English.
%- I'm not sure I understand Fig 5.1, is that earth in the cut out circle
%  thing? I also can't tell the difference between them...
%### 5.2
%- Fig 5.3 seems really important for the previous figures.

\todo{The dipole geometry isn't super new, but it's not widely-done. The height-resolved ionosphere is new and exciting! Ground signatures are new and exciting! }
