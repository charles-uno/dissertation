...
The bulk of the model is just electric and magnetic fields sloshing back and forth according to \farlaw, \amplaw, and Kirchhoff's formulation of \ohmlaw; respectively:
\begin{align}
  \ddt \vec{B} &= - \curl{E} &
  \tensor{\epsilon} \cdot \ddt \vec{E} &= \frac{1}{\mu_0} \curl{B} - \vec{J} &
  \vec{J} &= \tensor{\sigma} \cdot \vec{E}
\end{align}

Or, using \ohmlaw to eliminate the explicit current dependence in \amplaw, we can write:
\begin{align}
  \ddt \vec{B} &= - \curl{E} &
...
Neither the permittivity tensor nor the conductivity tensor couples the parallel and perpendicular components of \amplaw, so we can consider them separately. Let's start with the perpendicular components. 
\begin{align}
  \ep \ddt \vec{E}_\bot &= \frac{1}{\mu_0} \lr{ \curl{B} }_\bot - \tensor{\sigma}_\bot \cdot \vec{E}_\bot
\end{align}

If we cast this expression in terms of the linear polarizations $E_x$ and $E_y$, we find them coupled by the Hall conductivity. However, the conductivity tensor is be diagonal when we write this expression in terms of the circular polarizations. Taking $E_\pm \equiv \frac{1}{ \sqrt{2} } \lr{E_x \pm i E_y}$, and defining components of \curl{B} analogously,
\begin{align}
  \ep \ddt E_\pm &= \frac{1}{\mu_0} \lr{ \curl{B} }_\pm - \lr{ \sp \pm i \sh } E_\pm
\end{align}

The circularly-polarized expressions can be solved using integrating factors. Rotating back into \x-\y coordinates, then, the solution is: 
\begin{align}
  \label{eperp_tensor}
