...
Where $\tensor{ \mathbb{I} }$ is the identity tensor and in \x-\y-\z coordinates\footnote{Note the parallel component of the present definition of $\tensor{\Omega}$ differs slightly from that used in \cref{ch_math}, due to the latter's consideration of electron inertial effects.}, 
\begin{align}
...
\todo{Recall that the driving is folded into \vec{F} --- specifically, $F^2$. While in principle this couples it to both the poloidal and toroidal electric fields, in practice the driving is vanishingly small anywhere the Hall conductivity isn't. That is, driving is delivered to the poloidal electric field only. }
