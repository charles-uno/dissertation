...
\todo{Toroidal events exhibit a much sharper frequency distribution than poloidal events. (Maybe put the Gaussian fit back on here, even though it's not Gaussian, to estimate spread?) This makes sense --- toroidal events are supposed to depend sharply on $L$, while poloidal events are more smeared in $L$. Recall \cref{sec_day}. RBSP observations are concentrated over not that many $L$ shells. }
...
\todo{John says that the phase of these waves is one of the most interesting things about them. }

\todo{Phase tells us how much energy is in the standing wave, and how much is in the traveling wave. We can use it to estimate the timescale over which the wave's energy will be carried away by Poynting flux. Events are not selected based on phase, but it turns out that pretty much everything is a nice-looking standing wave. }

Taking $B$ and $E$ to be characteristic electric and magnetic field values, and $R$ to be a characteristic length scale, the energy in the system per unit volume, and the rate at which that energy is leaving the system, are given respectively by:
\begin{align}
  \label{def_phase}
  U &= \frac{R^3}{2\mz} B^2 &
...
%One such event is shown in \cref{fig_sample_event}. The left column shows electric and magnetic field waveforms in blue and red respectively. The right shows the corresponding spectra: imaginary Poynting flux in magenta (corrresponding to the strength of the standing wave) and real Poynting flux in green (for the traveling wave). The black line is a Gaussian fit to the magnitude of the Poynting flux. 

%This particular event shows a well-formed standing wave in the poloidal channel; the phase of \SI{79}{\degree} indicates that the standing wave exceeds the traveling wave by a factor of five. The coherent activity in the compressional magnetic field implies a low azimuthal modenumber, and thus a fast rotation of energy from the poloidal mode to the toroidal mode. In fact, that rotation may be the only thing keeping the toroidal mode going. Half of its energy is in the traveling wave as opposed to the standing wave (phase of \SI{130}{\degree}). With fields on the order of \SI{1}{\mV/\m} and \SI{1}{\nT}, and a characteristic length scale of \SI{e4}{\km} (the distance covered by a probe in half an hour at apogee), the energy should be carried away by Poynting flux on timescales of \SI{10}{\s}. 
