...
\todo{It's possible that the contour plots (\cref{sec_layers_p,sec_layers_t}) should go before the line plots (\cref{sec_lifetimes_day,sec_lifetimes_night}). They sorts depend on one another. Unclear if there's a better way to divide things up. }
...
% -----------------------------------------------------------------------------
% -----------------------------------------------------------------------------
\section{Spatial Distribution of Energy: Poloidal Mode}
  \label{sec_layers_p}
...
\cref{fig_layers_p_3_4_6} shows the radial distribution of poloidal energy on the nightside --- the same runs shown above in \cref{fig_U_3_4_6}. Broadly speaking, the behavior matches that shown in \cref{fig_layers_p_2_4_5,fig_layers_p_2_5_5}: energy is smeared across $L$-shells at small \azm and guided at high \azm, with particularly strong energy buildup when the drive frequency matches the local \Alfven frequency. 

\todo{Peak energies are relatively low, since dissipation timescales are faster than on the dayside. }
...
% -----------------------------------------------------------------------------
% -----------------------------------------------------------------------------
\section{Spatial Distribution of Energy: Toroidal Mode}
  \label{sec_layers_t}
...
\todo{On the nightside, the toroidal contours are just about as messy as the poloidal ones. }

\todo{Due to the resistive ionosphere, there's little toroidal activity at high \azm on the nightside. The rotation time is slow compared to the dissipation time. Recall from \cref{fig_U_3_4_6} that the toroidal energy is an order of magnitude smaller than the poloidal, or more. }
