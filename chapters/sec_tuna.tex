...
\todo{Zeroth order magnetic field. }
\begin{align}
  \vec{B}_0 &= \SI{3.1e4}{\nT} \lr{ \frac{R_E}{r} }^3 \lr{ 2 \cos\theta \, \hat{r} - \sin\theta \, \hat{\theta} }
\end{align}

\todo{Height-resolved conductivity profiles: \cref{fig_sigma}. }
\begin{figure}[!htb]
    \centering
...
\todo{Number density profile. }
\begin{align}
  n &= n_{AZ} \frac{ r_{AZ} }{r} + 
  n_{PS} \exp \arg{ \frac{-L}{ L_{PS} } } \lr{ \frac{1}{2} - \frac{1}{2} \tanh \frac{ L - L_{PP} }{ \dL_{PP} } }
\end{align}

\todo{Electric constant. Mean molecular mass $M$ should maybe be $m_p$ or $\bar{m}$ or something. }
\begin{align}
  \ep &= \epsilon_K + \frac{M n}{B_0^2}
\end{align}

\todo{\Alfven speed profile: \cref{fig_va}. }
\begin{figure}[!htb]
    \centering
