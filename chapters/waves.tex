...
%The first adiabatic invariant is the particle's magnetic moment, $\frac{m v_\bot^2}{2 B}$. 




%In the absence of other forces, a charged particle in a magnetic field obeys the Lorentz force law:
%\begin{align}
%  \frac{d}{dt} \vec{p} &= q \; \cross{v}{B}
%\end{align}
...
\todo{Pi2 pulsations are short duration ($\sim \SI{10}{\minute}$) waves with periods of a minute or two. They are excited on the nightside, and associated with substorm onset. The procecsses that give rise to Pi2 pulsations -- and how they are related to substorm onset -- remain controversial. }

ULF waves are characterized in terms of morphology, rather than in terms of any underlying physical processes. 

\todo{Pc1 and Pc2 pulsations are EMIC waves near the ion gyrofrequency. String-of-pearls pulsations are a subset which exhibits amplitude modulations on the order of \SI{10}{\mHz} (cause uncertain). Peak around L of 7. Guided. Sometimes also waveguide modes and cavity modes. May be filtered by the IAR, but generation happens further out, close to the equator. }

\todo{Pi1 may originate from current-driven turbulence in the upper ionosphere or near tail. }
...
\todo{``Much in the same way that a surfer gains energy by traveling just ahead of a wave in the ocean.'' }
...
\todo{Poloidal and toroidal, fundamental mode and first harmonic... make sure this vocabulary is inescapably clear! }
